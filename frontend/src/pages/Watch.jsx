  ...
  const handleEditName = async () => {
    ...
    try {
      const response = await axios.put(
        'http://127.0.0.1:5000/api/user/update-chat-name',
        { chatName: newName },
        { headers: { Authorization: `Bearer ${token}` } }
      );
    ...
    <div style={styles.container}>
        ...
        <div style={styles.headerRight}>
          {token && user ? (
            ...
            <>
              ...
              <div onClick={() => navigate('/auth')} style={styles.avatarClickable}>
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"></path><circle cx="12" cy="7" r="4"></circle></svg>
              </div>
            </>
          )}
      ...
      <main style={styles.mainContent}>
        ...
        <div style={styles.playerContainer}>
          <video 
            src="http://127.0.0.1:5000/api/video/stream" 
            controls 
            style={styles.videoPlayer}
            poster="https://via.placeholder.com/800x450/091172/ffffff?text=YADRO+Streaming"
          />
        </div>
        ...
        <div style={styles.sidebar}>
          ...
          <div style={styles.sidebarTabs}>
            <span 
              style={{...styles.sidebarTab, ...(activeTab === 'chat' ? styles.activeSidebarTab : {})}}
              onClick={() => setActiveTab('chat')}
            >
              Чат
            </span>
            <span 
              style={{...styles.sidebarTab, ...(activeTab === 'qa' ? styles.activeSidebarTab : {})}}
              ...
              <div style={styles.messagesList}>
                {messages.map((msg) => (
                  <div key={msg.id} style={styles.messageCard}>
                    ...
                    <div style={styles.likesBlock}>
                      <span style={styles.heartIcon}>♥</span>
                      <span style={styles.likesCount}>{msg.likes}</span>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div style={styles.sidebarFooter}>
                {token && user ? (
                  ...
                  <button onClick={() => navigate('/auth')} style={styles.guestBanner}>
                    Хотите отправить сообщение?<br />Кликните на эту кнопку
                  </button>
                )}
              </div>
            </div>
          ) : (
            <div style={styles.qaPlaceholder}>Раздел вопросов и ответов пока пуст.</div>
          )}
        </div>
